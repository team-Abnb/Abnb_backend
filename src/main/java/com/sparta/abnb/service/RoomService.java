...
public class RoomService {
    ...
    public RoomResponseDto createRoom(RoomRequestDto roomRequestDto, List<MultipartFile> multipartFiles, User user) {
        ...
        for (MultipartFile multipartFile : multipartFiles) {
            s3Util.uploadImage(multipartFile, ROOM_PICTURE_FOLDER);
        }
        // picture 등록 필요 ------------------------
    ...
    // 특정 방에 대한 수정 작업
    @Transactional
    public RoomResponseDto updateRoom(Long roomId, RoomRequestDto roomRequestDto, User user) throws AccessDeniedException {
        // 수정, 삭제 할 방이 존재하는지 확인
        Room room = findRoom(roomId);
        // 수정, 삭제 할 방의 권한을 확인
    ...
    public void checkAuthority(Room room, User user) throws AccessDeniedException {
        // admin 확인
        if (!user.getUserRole().getAuthority().equals("ROLE_ADMIN")) {
            // userId 확인
